...
export const SubmitButton = () => {
    ...
    const handleSubmit = async () => {
        if (nodes.length === 0) {
            setResult({ is_empty: true });
            return;
        }

        if (nodes.length > 0 && edges.length === 0) {
            setResult({ is_unwired: true });
            return;
        }
    ...
    return (
        <>
            ...
            >
                ...
                <span className="tracking-wide text-[17px] font-['Satoshi']">Submit Workflow</span>
            </button>
            ...
            {isLoading && !result && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#050505]/40 backdrop-blur-[4px] transition-opacity animate-in fade-in duration-300">
                    <div className="flex items-center gap-4 bg-[#18181B] border border-white/10 shadow-[0_30px_80px_rgba(0,0,0,0.8)] rounded-2xl px-6 py-4 animate-in slide-in-from-bottom-4 duration-300">
                        ...
                        <span className="text-gray-200 font-medium text-[16px] tracking-wide font-['Satoshi']">Analyzing workflow...</span>
                    </div>
                </div>
            )}
            ...
            {result && (
                <div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-[100] transition-opacity animate-in fade-in slide-in-from-bottom-12 duration-500">
                    <div className="w-[520px] bg-[#0A0A0B]/80 backdrop-blur-2xl border border-white/10 shadow-[0_30px_80px_rgba(0,0,0,0.8)] rounded-3xl p-5 flex items-center justify-between gap-5 relative overflow-hidden group">
                        ...
                        <div className={`absolute top-1/2 left-0 -translate-y-1/2 w-48 h-48 blur-[80px] opacity-20 pointer-events-none transition-colors duration-1000 ${result.is_empty || result.is_unwired ? 'bg-amber-500' : result.is_dag ? 'bg-emerald-500' : 'bg-red-500'}`}></div>

                        {/* LEFT SECTION: Large glowing icon */}
                        <div className="relative shrink-0 flex items-center justify-center p-1">
                            {result.is_empty || result.is_unwired ? (
                                <div className="relative w-14 h-14 rounded-full bg-amber-500/10 border border-amber-500/20 flex items-center justify-center shadow-[0_0_30px_rgba(245,158,11,0.2)]">
                                    <div className="absolute inset-0 rounded-full border border-amber-500/30 animate-pulse" style={{ animationDuration: '2s' }}></div>
                                    <AlertTriangle size={28} className="text-amber-400 drop-shadow-[0_0_8px_rgba(251,191,36,0.8)]" strokeWidth={2.5} />
                                </div>
                            ) : result.is_dag ? (
                        ...
                        <div className="flex-1 flex flex-col justify-center min-w-0 z-10 py-1">
                            <h2 className="text-[20px] font-bold text-white tracking-tight mb-1 font-['Satoshi']">
                                {result.is_empty ? 'Workflow Empty' : result.is_unwired ? 'Nodes Unconnected' : result.is_dag ? 'Execution Ready' : 'Deployment Blocked'}
                            </h2>
                            <p className="text-[15px] text-gray-400 leading-snug mb-3 pr-2">
                                {result.is_empty ? 'Your canvas is empty. Please drag and drop nodes from the toolbar to construct an AI pipeline before submitting.' : result.is_unwired ? 'You have added nodes to the canvas, but they are not connected. Please wire them together to create an execution flow.' : result.is_dag 
                            ...
                            {/* Metadata row */}
                            <div className="flex items-center gap-3 text-[12px] font-bold tracking-widest text-gray-500 uppercase">
                                {result.is_empty ? (
                                    <span className="flex items-center gap-1.5 text-amber-500/90"><AlertTriangle size={12} strokeWidth={3}/> Nodes: 0</span>
                                ) : result.is_unwired ? (
                                    <>
                                        <span className="flex items-center gap-1.5 text-amber-500/90"><CheckCircle2 size={12} strokeWidth={3}/> Nodes: {nodes.length}</span>
                                        <span className="flex items-center gap-1.5 text-amber-500/90"><AlertTriangle size={12} strokeWidth={3}/> Edges: 0</span>
                                    </>
                                ) : result.is_dag ? (
                                    <>
                        ...
                        <div className="shrink-0 flex flex-col gap-2 z-10">
                            {result.is_empty || result.is_unwired ? (
                                <button 
                                    ...
                                    className="px-6 py-2.5 rounded-full font-bold text-[15px] tracking-wide transition-all duration-300 flex items-center justify-center gap-2 bg-amber-500/10 text-amber-400 hover:bg-amber-500/20 border border-amber-500/30 hover:scale-[1.03] shadow-[0_0_20px_rgba(245,158,11,0.1)]"
                                >
                                    Dismiss
                                </button>
                            ) : result.is_dag ? (
                                <button 
                                    onClick={() => setResult(null)}
                                    className="px-6 py-2.5 rounded-full font-bold text-[15px] tracking-wide transition-all duration-300 flex items-center justify-center gap-2 bg-gradient-to-b from-emerald-400 to-emerald-600 text-[#061510] hover:from-emerald-300 hover:to-emerald-500 hover:scale-[1.03] shadow-[0_0_20px_rgba(16,185,129,0.2)] hover:shadow-[0_0_30px_rgba(16,185,129,0.4)]"
                                >
                                ...
                                <>
                                    <button 
                                        onClick={() => setResult(null)}
                                        className="px-6 py-2.5 rounded-full font-bold text-[15px] tracking-wide transition-all duration-300 flex items-center justify-center gap-2 bg-red-500/10 text-red-400 hover:bg-red-500/20 border border-red-500/30 hover:scale-[1.03] shadow-[0_0_20px_rgba(239,68,68,0.1)]"
                                    >
                                        Inspect Errors
                                    </button>
                                    <button 
                                        onClick={() => setResult(null)}
                                        className="text-[13px] text-gray-500 hover:text-gray-300 transition-colors font-medium text-center tracking-wide"
                                    >
                                        Auto Focus Cycle
                                    </button>
                                </>
